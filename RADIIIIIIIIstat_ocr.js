/* ============================================================
 Stat Screenshot OCR  v10 — fully local, zero external APIs
 ...
 2) TROOPS  (Troops Preview screenshot)
    ...
    • TG level detection — v10 COMPLETELY REWRITTEN:
        Old approach: flood-fill gold pixels → OCR raw crop → wrong results
        New approach: find gold badge circles that CONTAIN white digit pixels
          1. Flood-fill gold clusters in panel area
          2. Filter: clusters with white pixels inside (white/gold ratio ≥ 0.08)
          3. Filter: badge-sized (gold 80-2500px, bbox 12-60px wide)
          4. Crop badge bbox, render white→black on everything-else→white
          5. OCR with PSM 10 (single char) + PSM 7, majority vote
        If NO qualifying gold-with-white clusters → TG = 0 (no badge present)
...
(function () {
   ...
   * Pads the crop area slightly to ensure the full digit is captured.
   * Uses multiple PSM modes and crop paddings for robustness.
   *
   * @param {HTMLImageElement} img - Source image
   * @param {Object} badge - Badge candidate from findBadgeCandidates
   * @returns {Promise<number>} Digit 1-5, or 0 if unreadable
   */
  async function readBadgeDigitV10(img, badge) {
    const W = img.naturalWidth, H = img.naturalHeight;
    const results = [];

    // Try multiple padding levels around the badge bbox
    const paddings = [
      [0.3, 0.2],   // moderate pad
      [0.5, 0.3],   // larger pad
      [0.15, 0.1],  // tight pad
    ];
    ...
    for (const [padXFrac, padYFrac] of paddings) {
      ...
      // Create B&W canvas: white pixels → black (text), else → white (bg)
      // This isolates the white digit on the gold badge
      const scale = 8;
      ...
      // Convert: white pixels near gold → black (digit), everything else → white
      ...
      for (const psm of [10, 7, 8]) {
        try {
          const text = await runOCR(outCanvas, psm, '12345');
   ...
   *
   * v10: Uses white-inside-gold detection instead of raw gold flood-fill.
   *
   * @param {HTMLImageElement} img - Source image
  ...
  async function detectTGLevel(img, setStatus, panelY0px, panelY1px) {
    ...
    // Scan top 80% of panel (where the troop icons are)
    const scanY0 = panelY0px;
    const scanY1 = Math.round(panelY0px + (panelY1px - panelY0px) * 0.80);

    // Find badge candidates (gold clusters containing white pixels)
    const candidates = findBadgeCandidates(imgData, W, H, scanY0, scanY1);

    if (candidates.length === 0) {
      // No gold-with-white clusters found → no TG badge
      return 0;
    }

    // Sort by white ratio (higher = more likely a badge digit)
    // then by gold count (larger = more reliable)
    candidates.sort((a, b) => {
    ...
    // Try top 6 candidates
    const top = candidates.slice(0, 6);
    const allDigits = [];

    for (const badge of top) {
      const digit = await readBadgeDigitV10(img, badge);
      if (digit >= 1 && digit <= 5) {
    ...
    // All TG badges on a single screenshot should show the SAME digit.
    // Use majority vote across all successfully-read badges.
    const voteCounts = {};
    for (const d of allDigits) voteCounts[d] = (voteCounts[d] || 0) + 1;
