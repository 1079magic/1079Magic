...
function buildRally(fractions, rallySize, stock) {
  ...
  // Step 1: Allocate minimum infantry
  let inf = Math.min(stock.inf, iMin);

  // Step 2: Allocate minimum cavalry
  let cav = Math.min(stock.cav, cMin);

  // Step 3: Fill all remaining space with archers
  ...
  // Step 5: If still space, top up with extra infantry (up to iMax)
  ...
  function onOptimize() {
    ...
    // Update the bestReadout banner to show the ACTUAL fractions used in the table
    const bestReadoutEl = document.getElementById("bestReadout");
    if (bestReadoutEl) {
      bestReadoutEl.innerText =
        `Best Call Rally Composition ≈ ${usedDisp} (Inf/Cav/Arc) · [Inf 7.5–10%, Cav ≥ 10%].`;
    }
