...
(function(){
  ...
  function computePlots() {
    ...
    // bestReadout is set by onOptimize() with the actual used fractions — don't set it here

    updateRecommendedDisplay();
  }

// ---------- Rally Build (maximise archers: min inf + min cav first, then archers) ----------
function buildRally(fractions, rallySize, stock) {
  ...
  // Step 1: Start from requested fractions
  let inf = Math.round(fractions.fin * rallySize);
  let cav = Math.round(fractions.fcav * rallySize);
  let arc = rallySize - inf - cav;

  // Step 2: Clamp to game rules (inf 7.5–10%, cav ≥ 10%)
  inf = Math.max(iMin, Math.min(iMax, inf));
  cav = Math.max(cMin, cav);
  arc = rallySize - inf - cav;
  if (arc < 0) { cav = rallySize - inf; arc = 0; }

  // Step 3: Respect available stock
  inf = Math.min(inf, stock.inf);
  cav = Math.min(cav, stock.cav);
  arc = Math.min(arc, stock.arc);

  // Step 4: If stock ran short, fill remaining space — archers first, then cav, then inf
  let used = inf + cav + arc;
  if (used < rallySize) {
    const add = Math.min(rallySize - used, stock.arc - arc);
    arc += add; used += add;
  }
  if (used < rallySize) {
  ...
  function buildOptionAFormations(stock, formations, cap) {
    ...
    // This reserves the mandatory inf slots before anything else.
    ...
    // This reserves the mandatory cav slots before archers compete for space.
    ...
    // Remaining space per march = cap - infMin - cavMin.
  ...
  function onOptimize() {
    ...
    // Derive display string from what buildRally actually built (source of truth)
    const actualDisp = rallyTotal > 0
      ? formatTriplet(rally.inf / rallyTotal, rally.cav / rallyTotal, rally.arc / rallyTotal)
      : formatTriplet(usedFractions.fin, usedFractions.fcav, usedFractions.farc);

    const fracEl = document.getElementById("opt_fractionReadout");
    if (fracEl) fracEl.innerText = `Target fractions (bounded · Inf 7.5–10%, Cav ≥ 10%): ${actualDisp} · Best: ${bestDisp}`;

    // bestReadout banner always matches the CALL row in the table
