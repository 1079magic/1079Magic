...
(function () {
  ...
  function parseTroopText(text) {
    ...
    let seenFirstType = false; // don't bank header noise before first troop name
    let i = 0;
    ...
    while (i < lines.length) {
      ...
      // Not a type line — only bank numbers if we're already inside the troop
      // table (i.e. past the first type line). This prevents header noise like
      // "3/4  0/172.8K" from injecting garbage numbers into the bank before
      // the first "Apex Infantry / Apex Cavalry" line is seen.
      if (types.length === 0) {
        if (seenFirstType) numBank = numBank.concat(extractNums(line));
        i++;
        continue;
      }
      seenFirstType = true;
