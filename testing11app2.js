...
(function(){
  ...
  function getJoinFractionsManual() {
    ...
    // If field is empty → use last engine join fractions as fallback
    if (el.value.trim() === '') {
      if (hint) hint.textContent = "Using engine default (field cleared).";
      return null; // null = let engine use its own arc-first logic
    }
  ...
  function readbackCompositionFields(rally, joinPacks) {
    ...
    if (joinEl && joinPacks && joinPacks.length > 0) {
  ...
  function wireListeners(){
    ...
    if (compEl) {
      compEl.addEventListener("input", () => {
        // If user clears the field completely, reset to engine default on next compute
